 ...
 *         odd-length book with `showCover: false` that made the last leaf hard,
 ...
 *  - PC2: a one-page `showCover` book placed its cover on the LEFT half —
 ...
 * too: the `showCover` cases below pin the CURRENT behaviour (including the
...
describe('PC1 — a book with no covers gets no inferred hard leaf', () => {
  test('the last leaf of an odd-length book stays soft', () => {
    ...
    // inference used to harden. `showCover` is false, so the book has declared
    // that it has no covers.
    const { book: app } = book({ pageCount: 3, showCover: false, flippingTime: 0 });
  ...
  test('an even-length book is unchanged (it never had a singleton)', () => {
    const { book: app } = book({ pageCount: 4, showCover: false, flippingTime: 0 });
  ...
  test('an author-declared hard page is still hard (the inference is what changed)', () => {
    const { book: app, pages } = book({ pageCount: 3, showCover: false, flippingTime: 0 });
  ...
  test('showCover keeps its inferred back cover (no drift)', () => {
    // 6 pages + cover ⇒ spreads [0] [1,2] [3,4] [5]; page 5 is the singleton.
    const { book: app } = book({ pageCount: 6, showCover: true, flippingTime: 0 });
...
describe('PC2 — a single-leaf landscape spread is placed by what it IS', () => {
  test('a one-page showCover book draws its cover on the right half', () => {
