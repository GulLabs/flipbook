...
```js
await pageFlip.loadFromImages(['path/to/image1.jpg', 'path/to/image2.jpg']);
```
...
- `loadFromImages(imagesHref: string[]): Promise<void>` - Load pages from images (lazy canvas chunk)
