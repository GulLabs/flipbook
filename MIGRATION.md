...
- `PageFlip.flipNext` / `flipPrev` report a turn that cannot start as `false` plus a `turnRejected` event, carrying the engine's error code. A failure that is _not_ the engine's own (`PageFlipError`) — a broken renderer, a vanished node — still propagates rather than being hidden behind a refused turn. Explicit `turnToPage` / `flip` throw, including `PageFlipError('NOT_LOADED')` when called before a load — `flip` previously no-opped there.
