...
| `direction: 'rtl'`                            | New. Click, corner fold, drag and swipe all resolve the turn direction right-to-left; the fold still follows the pointer (coordinates are not mirrored). Programmatic `flipNext`/`flipPrev` follow page index.                                 |
