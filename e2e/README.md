...
## 3. Canvas pixels (`canvas.spec.ts`, `canvas-phase2.spec.ts`)

Real `getImageData` probes against `examples/vanilla/canvas.html` (never
`toDataURL`). Fixtures are generated by `pnpm fixtures:canvas` so expected
colours are source-derived.

| Suite                   | What it locks                                                                      |
| ----------------------- | ---------------------------------------------------------------------------------- |
| `canvas.spec.ts`        | Phase 0: spread paint, turns, portrait, reduced motion, destroy, DPR backing store |
| `canvas-phase2.spec.ts` | ADR 0001: contain/cover/fill, inset, blank leaves, 404/corrupt error path          |

Phase 2 tests **skip** when the engine does not yet expose `imageFit` /
descriptors (`body[data-fit=legacy-fill]` or `data-descriptors=0`). That is
deliberate: the assertions are the contract, written ahead of implementation.
When Phase 2 lands, the skips go away and a regression fails the suite.

Every visual assertion includes a **negative control** (probe outside the book
or “sum of channels > 0”) so a blank canvas cannot pass a paper-colour test.

```bash
pnpm fixtures:canvas
pnpm test:e2e:canvas
pnpm exec playwright test e2e/canvas-phase2.spec.ts
```

## 4. Gesture e2e (`gestures.spec.ts`) — §8.3
