...
The monorepo currently vendors the historical `page-flip` and `react-pageflip` packages. Scoped `@gullabs/*` package names, Changesets, and npm provenance are the target release path (same pattern as [gul-labs/any-llm](https://github.com/gul-labs/any-llm)).

Until that cutover lands:

1. Do not publish from a local machine unless it is an emergency.
2. Keep `repository.url` on every publishable `package.json` pointing at `https://github.com/gul-labs/flipbook.git`.
